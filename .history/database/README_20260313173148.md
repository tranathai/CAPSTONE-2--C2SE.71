...
- `schema_mysql.sql`: Tao database `mentori_grad`, bang `users`, `students`, `lecturers` va them du lieu mau.
...
- Dang ky: insert vao `users`, sau do insert vao `students` hoac `lecturers` theo `role`.
