...
export async function createNewTeam(req, res, next) {
  try {
    const { name, description, semester, leader_user_id } = req.body;

    if (!name || !name.trim()) {
      return res.status(400).json({ success: false, message: "Tên nhóm không được để trống" });
    }

    const teamId = await createTeam({ name: name.trim(), description, semester, leaderUserId: leader_user_id });
...
export async function updateExistingTeam(req, res, next) {
  try {
    ...
    await updateTeam(teamId, { name, description, semester, leaderUserId: leader_user_id });
...
export async function addMember(req, res, next) {
  try {
    ...
    await addTeamMember(teamId, user_id, is_leader);
