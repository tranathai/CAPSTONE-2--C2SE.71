...
export default function StudentTeam() {
  ...
  useEffect(() => {
    teams.myTeam().then((t) => { setTeam(t); setLoading(false); }).catch(() => setLoading(false));
  }, []);
  ...
  return (
    <div className="page-container">
      <div className="page-header">
        <h1>Danh bạ nhóm</h1>
        <p>Thông tin nhóm và các thành viên</p>
      </div>
        ...
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
          <div>
            ...
            <p style={{ color: "#64748b", fontSize: "0.875rem", marginTop: 2 }}>
              {team.semester ? `Học kỳ: ${team.semester}` : ""} {team.leader_name ? `• Trưởng nhóm: ${team.leader_name}` : ""}
            </p>
      ...
      <div className="card">
        <div className="card-title"><Icon name="Users" size={14} sx={{ marginRight: 6 }} />Thành viên ({team.members?.length || 0})</div>
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(260px, 1fr))", gap: 12 }}>
          {team.members?.map((m) => (
            <div key={m.id} style={{ background: "#f8fafc", borderRadius: 10, padding: 16, border: "1px solid #e2e8f0" }}>
              <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 10 }}>
                ...
                <div>
                  <div style={{ fontWeight: 700, fontSize: "0.95rem", display: "flex", alignItems: "center", gap: 6 }}>
                    {m.full_name}
                    {m.is_leader && <Icon name="Star" size={14} color="#f59e0b" sx={{ color: "#f59e0b", fill: "#f59e0b" }} />}
                  </div>
